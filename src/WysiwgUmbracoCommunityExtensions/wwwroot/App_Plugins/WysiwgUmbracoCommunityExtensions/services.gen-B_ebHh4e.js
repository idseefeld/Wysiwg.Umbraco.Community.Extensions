...
var q = /\{[^{}]+\}/g, w = ({ allowReserved: l, name: e, value: a }) => {
  ...
  let n = A(i), r = s.map((t) => i === "label" || i === "simple" ? l ? t : encodeURIComponent(t) : w({ allowReserved: l, name: a, value: t })).join(n);
  ...
  let n = W(i), r = Object.entries(s).map(([t, u]) => w({ allowReserved: l, name: i === "deepObject" ? `${a}[${t}]` : t, value: u })).join(n);
  return i === "label" || i === "matrix" ? n + r : r;
}, E = ({ path: l, url: e }) => {
  let a = e, i = e.match(q);
  if (i) for (let s of i) {
  ...
  return a;
}, U = ({ allowReserved: l, array: e, object: a } = {}) => (i) => {
  let s = [];
  ...
  var i;
  let a = { ...l, ...e };
  return (i = a.baseUrl) != null && i.endsWith("/") && (a.baseUrl = a.baseUrl.substring(0, a.baseUrl.length - 1)), a.headers = $(l.headers, e.headers), a;
}, $ = (...l) => {
  let e = new Headers();
  for (let a of l) {
...
}, D = () => ({ error: new p(), request: new p(), response: new p() }), N = { bodySerializer: (l) => JSON.stringify(l) }, P = U({ allowReserved: !1, array: { explode: !0, style: "form" }, object: { explode: !0, style: "deepObject" } }), H = { "Content-Type": "application/json" }, C = (l = {}) => ({ ...N, baseUrl: "", fetch: globalThis.fetch, headers: H, parseAs: "auto", querySerializer: P, ...l }), J = (l = {}) => {
  let e = x(C(), l), a = () => ({ ...e }), i = (r) => (e = x(e, r), a()), s = D(), n = async (r) => {
    let t = { ...e, ...r, headers: $(e.headers, r.headers) };
    t.body && t.bodySerializer && (t.body = t.bodySerializer(t.body)), t.body || t.headers.delete("Content-Type");
    let u = I({ baseUrl: t.baseUrl ?? "", path: t.path, query: t.query, querySerializer: typeof t.querySerializer == "function" ? t.querySerializer : U(t.querySerializer), url: t.url }), d = { redirect: "follow", ...t }, o = new Request(u, d);
    for (let f of s.request._fns) o = await f(o, t);
    let O = t.fetch, c = await O(o);
    for (let f of s.response._fns) c = await f(c, o, t);
    ...
    if (c.ok) {
      ...
      let f = (t.parseAs === "auto" ? z(c.headers.get("Content-Type")) : t.parseAs) ?? "json", g = await c[f]();
    ...
    let y = h;
    for (let f of s.error._fns) y = await f(h, c, o, t);
...
//# sourceMappingURL=services.gen-B_ebHh4e.js.map
