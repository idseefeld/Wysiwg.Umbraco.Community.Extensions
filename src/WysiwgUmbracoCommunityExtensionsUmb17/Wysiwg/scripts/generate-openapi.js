...
if (openApiUrl === undefined) {
  ...
  console.error(`Example: node generate-openapi.js ${chalk.yellow('https://localhost:[PORT]/umbraco/openApi/REPLACE_ME.json')}`);
  process.exit();
}
  ...
  .then(openApiSpec => {
    ...
    const filteredSpec = {
      ...
      paths: Object.keys(openApiSpec.paths)
        .filter(path => path.startsWith('/api/v1/wysiwg/')) // Replace with your desired path filter
        .reduce((obj, key) => {
