...
const Q = (e) => {
  ...
  if (!t) {
    const s = (e ? o : o.map((i) => encodeURIComponent(i))).join(K(a));
    switch (a) {
      case "label":
        return `.${s}`;
      case "matrix":
  ...
  const c = Q(a), n = o.map((s) => a === "label" || a === "simple" ? e ? s : encodeURIComponent(s) : k({
    allowReserved: e,
    name: r,
    value: s
  })).join(c);
  ...
  if (o instanceof Date)
    return c ? o.toISOString() : `${r}=${o.toISOString()}`;
  if (a !== "deepObject" && !t) {
    let i = [];
    Object.entries(o).forEach(([f, l]) => {
      i = [...i, f, e ? l : encodeURIComponent(l)];
    });
    const m = i.join(",");
    switch (a) {
      case "form":
  ...
  const n = X(a), s = Object.entries(o).map(
    ([i, m]) => k({
      allowReserved: e,
      name: a === "deepObject" ? `${r}[${i}]` : i,
      value: m
    })
  ).join(n);
  ...
  if (a)
    for (const o of a) {
      ...
      if (i == null)
        continue;
      if (Array.isArray(i)) {
        r = r.replace(o, D({ explode: c, name: n, style: s, value: i }));
        continue;
      }
      if (typeof i == "object") {
        r = r.replace(
...
const ie = () => ({
  ...
  let t = R(V(), e);
  const r = () => ({ ...t }), a = (f) => (t = R(t, f), r()), o = ie(), c = async (f) => {
    const l = {
      ...t,
      ...f,
  ...
  }, s = (f) => (l) => n({ ...l, method: f }), i = (f) => async (l) => {
...
//# sourceMappingURL=client.gen-Cmrrvp-8.js.map
