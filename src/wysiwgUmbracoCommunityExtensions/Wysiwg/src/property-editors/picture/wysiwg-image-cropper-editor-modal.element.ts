...
import {
  WysiwgImageCropperEditorModalData,
  WysiwgImageCropperEditorModalValue,
} from "./wysiwg-image-cropper-editor-modal.token";
import {
...
import { WysiwgInputImageCropperFieldElement } from "./wysiwg-image-cropper-field.element";

const elementName = "wysiwg-image-cropper-editor-modal";

@customElement(elementName)
export class WysiwgImageCropperEditorModalElement extends UmbModalBaseElement<
...
> {
  ...
  async #getSrc() {
    ...
    const crops: Array<UmbCropModel> = this._crops.map((crop) => {
      const existingCrop = this.value.crops?.find(
        (c) => c.alias === crop.alias
      );
  ...
  #onChange(e: CustomEvent & { target: WysiwgInputImageCropperFieldElement }) {
  ...
  override render() {
    return html`
      <umb-body-layout
        headline=${this.localize.term("defaultdialogs_selectMedia")}
      >
        ${this.#renderBody()}
        <div slot="actions">
          <uui-button
        ...
        >
          <div id="actions" slot="actions">
            <uui-button
              ...
              label=${this.localize.term("mediaPicker_changeMedia")}
            >
              <uui-icon name="icon-search"></uui-icon>${this.localize.term(
            ...
            >
              <uui-icon name="icon-out"></uui-icon>${this.localize.term(
...
export default WysiwgImageCropperEditorModalElement;

declare global {
  interface HTMLElementTagNameMap {
    [elementName]: WysiwgImageCropperEditorModalElement;
  }
}
